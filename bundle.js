...
function ordenar() {
  ...
  var algoritmo = $("#selectAlgoritmo").val();
  orderBy(items, algoritmos[algoritmo]);
}

function orderBy(array, algoritmo) {
  start = Date.now();
  var result = algoritmo(array, 0, array.length - 1);
  ...
  ;
  var tempo = Date.now() - start;
  var milisegundos = document.createTextNode("Tempo em milisegundos: " + tempo.toString());
  $("h1").text('');
...
function selectionSort(items) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    min = i; //check the rest of the array to see if anything is smaller
